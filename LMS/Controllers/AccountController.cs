...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            // Select the highest uid from student, professor, and admin table.
            // Since StudentID, ProfessorID, and AdminID are auto_increment, so it will be 
            // the same as uID except the 'u' heading, 
            // then we can use it for comparison since it's int, but uID is char.
            var AlluIDDb = (from student in db.Students
                            orderby student.StudentId descending
                            select student.UId).Take(1).Union(
                           (from professor in db.Professors
                            orderby professor.ProfessorId descending
                            select professor.UId).Take(1).Union(
                           (from admin in db.Administrators
                            orderby admin.AdminId descending
                            select admin.UId).Take(1)));

            // If it's not emtpy, then find the biggest uID.
            int biggestUID = -1;
            if (AlluIDDb.Count() > 0)
            {
                int tempUID = 0;
                foreach (string uid in AlluIDDb)
                {
                    tempUID = int.Parse(uid.Substring(1));
                    if (tempUID > biggestUID)
                    {
                        biggestUID = tempUID;
                    }
                }
                UserUID = "u" + (biggestUID+1).ToString("D7");
            ...
            uint DepartmentID = (from department in db.Departments where department.Subject == SubjectAbbrev select department.DepartmentId).First();
